...
#Kinematic viscoity as a function os speed in order to dissipate and avoid reverse flow in the outlet boundary
ν_value = 0.001 # [m2/s] kinematic viscosity
...
ν = zeros(num_cells(model))
cell_nodes = get_cell_node_ids(model)
n_coord = Gridap.Geometry.get_node_coordinates(model)
L = 8 #[m] Length of the domain in the x direction
t = 0.85 #from 0.8 * L the viscosity increase, so the final part dissipate everything and no reverse flow problem
for i = 1:1:num_cells(model)
  x_cell = n_coord[cell_nodes[i][1]][1]
    for j = 2:1:4
      if x_cell > n_coord[cell_nodes[i][j]][1]
        x_cell = n_coord[cell_nodes[i][j]][1]
    end
  end
  
  if x_cell>L*t
    ν[i] = ν_value + ((x_cell-L*t)/((1-t)*L))^2
  else
    ν[i] = ν_value
...
stab_equations(t, (u, p), (v, q)) = ∫((τ ∘ (u, h, ν) * (u ⋅ ∇(v) + ∇(q))) ⊙ Rm(t, (u, p)) # First term: SUPG, second term: PSPG
                                      +
                                      τb ∘ (u, h, ν) * (∇ ⋅ v) ⊙ Rc(u) # Bulk viscosity. Try commenting out both stabilization terms to see what happens in periodic and non-periodic cases
)dΩ
...
writevtk(Ω, "initial_condition", cellfields=["uh" => uh0, "ph" => ph0, "nu"=>ν])


#For extracting results, for fft

N_samples = 100
U_vector = zeros(N_samples)
