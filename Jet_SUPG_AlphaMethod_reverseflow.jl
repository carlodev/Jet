...
ν(x, u) = (norm.(v1(x)) > 3.5) ? 100 : nu_value
...
#τb(u,h) = (u⋅u)*τ(u,h)
τb(u,h,x) = (u⋅u)*τ(u,h,x)

var_equations(t,(u,p),(v,q)) = ∫(
  ν(x,u)*∇(v)⊙∇(u) # Viscous term
    + v⊙Rm(t,(u,p)) # Other momentum terms
    + q*Rc(u)
 )dΩ # Continuity


stab_equations(t,(u,p),(v,q)) = ∫(  (τ∘(u,h,x)*(u⋅∇(v) + ∇(q)))⊙Rm(t,(u,p)) # First term: SUPG, second term: PSPG
    +τb∘(u,h,x)*(∇⋅v)⊙Rc(u) # Bulk viscosity. Try commenting out both stabilization terms to see what happens in periodic and non-periodic cases
)dΩ


res(t,x,(u,p),(v,q)) = var_equations(t,(u,p),(v,q)) + stab_equations(t,(u,p),(v,q))
